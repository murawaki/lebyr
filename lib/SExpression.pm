...
SExpression -- Parse Lisp S-Expressions into perl data
structures.
...
=head1 SYNOPSIS
    ...
    $ds = SExpression->new({fold_alists => 1});
...
Returns a new SExpression object. Possibly args are:
...
If true, symbols become instances of L<SExpression::Symbol>
instead of globrefs.
...
sub new {
    ...
    my $parser = SExpression::Parser->new;
...
=item Conses become SExpression::Cons objects

See L<SExpression::Cons> for how to deal with these. See also
...
perl. You can ask for any of these in the export list, e.g.
    ...
    use SExpression qw(cons consp);

=head2 cons CAR CDR

Convenience method for SExpression::Cons->new(CAR, CDR)

=cut

sub cons ($$) {
    my ($car, $cdr) = @_;
    return SExpression::Cons->new($car, $cdr);
}
...
sub consp ($) {
    my $thing = shift;
    return ref($thing) && UNIVERSAL::isa($thing, 'SExpression::Cons');
}
...
=head1 SExpression::Parser callbacks
...
sub new_symbol {
    ...
    if($self->get_use_symbol_class) {
        return SExpression::Symbol->new($name);
    } else {
